...
final class ChatViewModel: ViewModel {
	...
	private var totalChatMessages: [ChatRepresentable] = []
	...
	init(configuration: ConfigurationModel) {
		...
		let totalChatMessages = generateMessageSamples(1000)
		let chatRepresentableMessages = convertToChatRepresentable(totalChatMessages)
		self.totalChatMessages = chatRepresentableMessages
		currentPage = 0
		chatMessages = Array(self.totalChatMessages[0 ..< configuration.loadMoreCount])
	}
	...
	func loadMore() {
		...
		DispatchQueue.main.asyncAfter(deadline: .now() + 0.3) { [weak self] in
			...
			currentPage += 1
			let lowerBound = currentPage * configuration.loadMoreCount
			let upperBound = lowerBound + configuration.loadMoreCount
			chatMessages.append(contentsOf: Array(self.totalChatMessages[lowerBound ..< upperBound]))
			canLoadMore = true
		}
	}
	
	private func convertToChatRepresentable(_ totalChatMessages: [Message]) -> [ChatRepresentable]{
		let chatRepresentableMessages: [ChatRepresentable] = totalChatMessages.map { chatMessage in
		...
		for i in 1...count {
			...
			let chatMessage = Message(id: i,
									  ...
									  image: Photo(imageUrl: URL(string: imageUrl),
												   width: width,
												   height: height,
												   // assume thumb image is sent via socket
									  ...
									  sender: sender)
			totalChatMessages.append(chatMessage)
		}
