...
final class ReceivedImageTextCell: UICollectionViewCell {
	...
	func update(with imageTextMessage: ChatReceivedImageTextMessage, configuration: ConfigurationModel) {
		...
		senderNameLabel.text = imageTextMessage.sender.name
		senderNameLabel.font = .systemFont(ofSize: configuration.textSize - 2)
		
		URLSession.shared.dataTaskPublisher(for: imageTextMessage.sender.imageUrl)
		...
		if let cachedImage = ImageCache.shared.getImage(forKey: "\(imageTextMessage.image.imageUrl.path))_resized") {
			imageView.image = cachedImage.resizeImage(targetSize: imageView.frame.size)
		} else {
			URLSession.shared.dataTaskPublisher(for: imageTextMessage.image.imageUrl)
				.map { UIImage(data: $0.data) }
				.replaceError(with: nil)
				.receive(on: DispatchQueue.main)
				.sink(receiveValue: { [weak self] image in
					...
					if let image,
						let resizedImage {
						ImageCache.shared.setImage(image, forKey: imageTextMessage.image.imageUrl.path)
						ImageCache.shared.setImage(resizedImage, forKey: "\(imageTextMessage.image.imageUrl.path))_resized")
					}
		...
		let ratio = CGFloat(imageTextMessage.image.height) / CGFloat(imageTextMessage.image.width)
		...
		if imageView.image == nil {
			imageView.image = UIImage(data: imageTextMessage.image.thumbImageData)
		}
